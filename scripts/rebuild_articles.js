...
const PDF_DATES = fs.existsSync(path.join(ROOT, 'extracted', 'pdf_dates.json'))
  ? JSON.parse(fs.readFileSync(path.join(ROOT, 'extracted', 'pdf_dates.json'), 'utf8'))
  : {};
...
for (const [num, title_he, title_en, fallback_date, source] of ARTICLES_META) {
  ...
  // Prefer the date written inside the PDF over the email/fallback date.
  const date = PDF_DATES[String(num)] || fallback_date;
  const parts = [
    `num: ${num}`,
